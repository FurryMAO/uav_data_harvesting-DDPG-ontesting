...
def eval_logs(event_path):
    ...
    _, _, vals = zip(*event_acc.Tensors('cumulative_reward'))
    cumulative_reward = [tf.make_ndarray(val) for val in vals]
    ...
    print('accumulative rewards:',sum(cumulative_reward)/ len(cumulative_reward))
