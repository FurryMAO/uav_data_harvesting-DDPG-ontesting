...
class GridRewards:
    ...
    def calculate_motion_rewards(self, state, action: ContinueAction, next_state):
        reward = 0.0
        ...
        if state.position == next_state.position and not next_state.landed and not (action.dx==0 and action.dy==0):
